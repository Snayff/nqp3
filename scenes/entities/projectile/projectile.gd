...
var target_pos : Vector2  ## target position. ignored if there is a target actor.  N.B. not implemented.
var has_physicality : bool = false  ## applies physics, e.g. knockback  N.B. not implemented.
var is_homing : bool = false  ## whether projectile alters direction to track target. does nothing if moving to target_pos.
var hits_before_expiry : int = 1  ## expires after this many hits. INF to never expire from hits. N.B. not implemented.
